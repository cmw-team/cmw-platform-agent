...
class CmwAgent:
    ...
    async def stream_chat(
        ...
        try:
            ...
            print(
                f"📊 Tracking stats - LLM: {llm_type}, Session: {self.session_id}, Response time: {response_time:.2f}s"
            )
            if self.stats_manager:
                ...
                # Track conversation with full model name
                full_model_name = (
                    f"{llm_type}/{self.llm_instance.model_name}"
                    if self.llm_instance
                    else llm_type
                )
            ...
            if self.stats_manager:
                ...
                self.stats_manager.track_error(error_type=str(e), llm_type=llm_type)
