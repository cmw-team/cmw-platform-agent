...
class EditOrCreateBooleanAttributeSchema(BaseModel):
...
class GetBooleanAttributeSchema(BaseModel):
    application_system_name: str = Field(
        description=(
            "System name of the application with the template where the attribute is created. "
            "Рус: 'Системное имя приложения'"
...
@tool("get_boolean_attribute", return_direct=False, args_schema=GetBooleanAttributeSchema)
def get_boolean_attribute(
    ...
    """
    Get a boolean attribute by its `system_name` within a given `template_system_name` and `application_system_name`.
    ...
    keys_to_remove = ['format', 'isUnique', 'isIndexed', 'isTitle', 'isMultiValue', 'isMandatory', 'isOwnership', 'instanceGlobalAlias', 'imageColorType', 'imagePreserveAspectRatio']
