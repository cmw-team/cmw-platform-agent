...
class EditOrCreateDrawingAttributeSchema(BaseModel):
...
class GetDrawingAttributeSchema(BaseModel):
    application_system_name: str = Field(
        description=(
            "System name of the application with the template where the attribute is created. "
            "Рус: 'Системное имя приложения'"
...
@tool("get_drawing_attribute", return_direct=False, args_schema=GetDrawingAttributeSchema)
def get_drawing_attribute(
    ...
    """
    Get a drawing attribute by its `system_name` within a given `template_system_name` and `application_system_name`.
